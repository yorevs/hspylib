...
class TestAppConfig(unittest.TestCase):
    ...
    def test_should_get_a_weird_valued_property(self):
        ...
        self.assertEqual(expected_value, self.configs["test.weird.property"])

    def test_should_get_the_overridden_value(self):
        expected_value = "yes its overridden"
        self.assertEqual(expected_value, self.configs["test.overridden.by.environ"])

    def test_should_get_int_property(self):
        expected_value = 1055
        self.assertEqual(expected_value, self.configs.get_int("test.int.property"))

    def test_should_get_float_property(self):
        expected_value = 3.14
        self.assertEqual(expected_value, self.configs.get_float("test.float.property"))

    def test_should_get_bool_property(self):
        expected_value_1 = False
        expected_value_2 = True
        self.assertEqual(expected_value_1, self.configs.get_bool("test.bool.property1"))
        self.assertEqual(expected_value_2, self.configs.get_bool("test.bool.property2"))

    def test_should_fail_due_to_invalid_type(self):
        with self.assertRaises(ValueError) as context:
            self.configs.get_int("test.overridden.by.environ")
        self.assertTrue("invalid literal for int() with base 10" in str(context.exception))

    def test_should_be_subscriptable(self):
        expected_value_1 = "FALse"
        expected_value_2 = "TRue"
        self.assertEqual(expected_value_1, self.configs["test.bool.property1"])
        self.assertEqual(expected_value_2, self.configs["test.bool.property2"])
    ...
    def test_should_replace_placeholders(self):
        ...
        self.assertEqual(
            expected_value_1, self.configs.get_int("test.placeholder.int.property", placeholders))
        self.assertEqual(
            expected_value_2, self.configs.get_float("test.placeholder.float.property", placeholders))
        self.assertEqual(
            expected_value_3, self.configs.get_bool("test.placeholder.bool.property", placeholders))
        self.assertEqual(
            expected_value_4, self.configs.get("test.placeholder.strings.property", placeholders))
    ...
    def test_should_replace_placeholder_using_environment_vars(self):
        ...
        self.assertEqual(
            expected_value, self.configs.get("test.placeholder.environment"))
