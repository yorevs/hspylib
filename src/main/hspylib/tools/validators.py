...
def validate_string(string: str, pattern: str, min_len: int = 0, max_len: int = 255):
    str_len = len(string)
    return min_len <= str_len <= max_len and bool(re.match(pattern, string))


def validate_int(number: str, min_value: int = 0, max_value: int = 65535):
    return number.isdigit() and min_value <= int(number) <= max_value


def validate_float(number: str, min_value: float = 0, max_value: float = 65535):
